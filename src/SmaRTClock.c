...
void RTC_Init (void)
{
   ...
   if(RTC0KEY == RTC0KEY_RTC0ST__LOCKED) {
   ...
   //----------------------------------------  
      ...
      // Configure SmaRTClock Pin Configuration (RTC0PIN):
      // 	Short XTAL3 and XTAL4 together to enable internal oscillator
      RTC_Write (RTC0PIN, RTC0PIN_RTCPIN__SELF_OSCILLATE);
      
      // Configure SmaRTClock Oscillator Control (RTC0XCN):
      //	Automatic gain control disabled (must be disabled for self-oscillate mode: page 208)
      //	Self-oscillate mode selected
      //	Disable double bias (This sets the RTC oscillator to ~20kHz. Enabling bias doubling sets the RTC
      //						 oscillator to ~40kHz. Fine-tune the frequency by changing the LOAD_CAP value
      //						 in SmaRTClock.h. Disabling double bias roughly halves the current pulled in Sleep mode.)
      // Note: Make sure that the Missing SmaRTClock Detector is disabled before modifying RTC0XCN (see page 210).
      // The Missing SmaRTClock Detector is disabled on reset so is not modified here.
      RTC_Write (RTC0XCN, RTC0XCN_AGCEN__DISABLED | RTC0XCN_XMODE__SELF_OSCILLATE | RTC0XCN_BIASX2__DISABLED);
                                      
      // Configure SmaRTClock Oscillator Configuration (RTC0XCF):
      //	Disable Auto Load Cap Stepping
      // 	Set the Load Capacitance Value
      RTC_Write (RTC0XCF, (RTC0XCF_AUTOSTP__DISABLED | LOADCAP_VALUE));
   ...
   // Wait for the SmaRTClock oscillator crystal valid detector to indicate that the
   // oscillations are stable (section 20.2.6). This can be done by reading the CLKVLD bit (RTX0XCN.4).
   // However, the CLKVLD bit has a bit has a blanking interval of 2 ms. During the first
   // 2ms after turning on the crystal oscillator, the output of CLKVLD is not valid.
   // To delay by 2ms, switch lower power clock to 156 kHz, run loop, switch clock back.
   CLKSEL_save = CLKSEL;
   CLKSEL         =  CLKSEL_CLKDIV__SYSCLK_DIV_128 | CLKSEL_CLKSL__LPOSC;
   for (i=0x340; i!=0; i--);
   // Restore system clock and poll System Clock Divider Clock Ready Flag
   while(!(CLKSEL & CLKSEL_CLKRDY__SET));
   CLKSEL = CLKSEL_save;

   // Configure SmaRTClock CONTROL (RTC0CN):
   //	SmaRTClock Enable
   //	Missing SmaRTClock Detector Disable
   //	SmaRTClock Timer Run Control Stopped
   //	SmaRTClock Alarm Enable
   //	SmaRTClock Alarm Event Flag Enable Auto Reset
   RTC0CN_Local = RTC0CN_RTC0EN__ENABLED | RTC0CN_MCLKEN__DISABLED | RTC0CN_RTC0TR__STOP
		    | RTC0CN_RTC0AEN__ENABLED | RTC0CN_ALRM__SET;
   RTC_Write (RTC0CN, RTC0CN_Local);


   PMU0CF = PMU0CF_CLEAR__ALL_FLAGS; // Clear PMU wake-up source flags <-- NECESSARY?
}
